...
describe('new_post_path', () => {
  ...
  const newPostPath =
      require('../../../lib/plugins/filter/new_post_path').bind(hexo);
  ...
  it('default layout + slug', async () => {
    ...
    const target = await newPostPath({slug : 'foo'});
  ...
  it('date', async () => {
    ...
    const target = await newPostPath({slug : 'foo', date : date.toDate()});
  ...
  it('extra data', async () => {
    ...
    const target = await newPostPath({slug : 'foo', foo : 'oh', bar : 'ya'});
  ...
  it('replace existing files', async () => {
    ...
    const target = await newPostPath({path : filename}, true);
  ...
  it('rename if target existed', async () => {
    ...
    const target = await newPostPath({path : filename[0]});
  ...
  it('data is required', async () => {
    try {
      ...
      err.message.includes('Either data.path or data.slug is required!')
          .should.eql(true);
    }
  });
});
