...
describe('Hexo', () => {
  ...
  function checkStream(stream, expected) {
    return testUtil.stream.read(stream).then(
        data => { data.should.eql(expected); });
  }

  function loadAssetGenerator() {
    hexo.extend.generator.register(
        'asset', require('../../../lib/plugins/generator/asset'));
  }
  ...
  it('constructs mutli-config', () => {
    const configs =
        [ '../../../fixtures/_config.json', '../../../fixtures/_config.json' ];
  ...
  it('call()', () => hexo.call('test', {foo : 'bar'})
                         .then(data => { data.should.eql({foo : 'bar'}); }));
  ...
  it('call() - console not registered', () => {
    const errorCallback = sinon.spy(err => {
      err.should.have.property(
          'message', 'Console `nothing` has not been registered yet!');
    });

    return hexo.call('nothing')
        .catch(errorCallback)
  ...
  it('init()', () => {
    ...
    return hexo.init().then(() => { hook.calledOnce.should.be.true; });
  });
  ...
  it('exit() - error handling - promise', () => {
    return Promise.all([
      hexo.exit({foo : 'bar'}), new Promise((resolve, reject) => {
        hexo.once('exit', err => {
          try {
            err.should.eql({foo : 'bar'});
            resolve();
  ...
  it('future posts',
     () => Post.insert([
               ...
               .then(posts => {
                 ...
                 hexo.locals.invalidate();
                 hexo.locals.get('posts').map(mapper).should.eql(
                     posts.map(mapper));
  ...
  it('future pages',
     () => Page.insert([
               ...
               .then(pages => {
                 function mapper(page) { return page._id; }

                 // future on
                 hexo.config.future = true;
                 hexo.locals.invalidate();
                 hexo.locals.get('pages').map(mapper).should.eql(
                     pages.map(mapper));
  ...
  it('_generate()', () => {
    ...
    const beforeHook = sinon.spy(() => { hexo.locals.set('test', 'foo'); });
  ...
  it('_generate() - layout', () => {
    hexo.theme.setView('test.swig', [
      '{{ config.title }}', '{{ page.foo }}', '{{ layout }}', '{{ view_dir }}'
    ].join('\n'));
    ...
    const expected = [
      hexo.config.title, 'bar', 'layout',
      pathFn.join(hexo.theme_dir, 'layout') + pathFn.sep
    ].join('\n');

    return hexo._generate().then(() =>
                                     checkStream(route.get('test'), expected));
  });
  ...
  it('_generate() - layout array', () => {
    ...
    hexo.extend.generator.register(
        'test', () => ({path : 'test', layout : [ 'foo', 'bar', 'baz' ]}));
  ...
  it('_generate() - validate locals', () => {
    ...
    return hexo._generate().then(
        () => checkStream(route.get('test'), [
          'test', hexo.config.url + '/test',
          pathFn.join(hexo.theme_dir, 'layout') + pathFn.sep
        ].join('\n')));
  });
  ...
  it('_generate() - should encode url', () => {
    ...
    hexo.extend.generator.register('test', () => ({path, layout : 'test'}));

    return hexo._generate().then(
        () => checkStream(route.get(path), full_url_for.call(hexo, path)));
  });
  ...
  it('_generate() - reset cache for new route', () => {
    ...
    hexo.extend.generator.register(
        'test',
        () =>
            ({path : 'test', layout : 'test', data : {count : () => count++}}));

    // First generation
    return hexo._generate({cache : true})
        .then(() => checkStream(route.get('test'), '0'))
        .then(() => checkStream(route.get('test'),
                                '0')) // should return cached result
  ...
  it('execFilter()', () => {
    ...
    return hexo.execFilter('exec_test', '')
        .then(result => { result.should.eql('foo'); });
  });
