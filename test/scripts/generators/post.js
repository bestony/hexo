...
describe('post', () => {
  ...
  const generator =
      Promise.method(require('../../../lib/plugins/generator/post').bind(hexo));
  ...
  it('default layout', async () => {
    const post = await Post.insert({source : 'foo', slug : 'bar'});
    const data = await generator(locals());
    post.__post = true;

    data.should.eql(
        [ {path : 'bar/', layout : [ 'post', 'page', 'index' ], data : post} ]);
  ...
  it('custom layout', async () => {
    const post =
        await Post.insert({source : 'foo', slug : 'bar', layout : 'photo'});
    const data = await generator(locals());
  ...
  it('layout disabled', async () => {
    const post =
        await Post.insert({source : 'foo', slug : 'bar', layout : false});
    const data = await generator(locals());
