...
describe('Post', () => {
  ...
  it('default values', () => {
    ...
    return Post.insert({source : 'foo.md', slug : 'bar'}).then(data => {
  ...
  it('slug - required', () => {
    const errorCallback = sinon.spy(
        err => { err.should.have.property('message', '`slug` is required!'); });

    return Post.insert({source : 'foo.md'})
        .catch(errorCallback)
        .finally(() => { errorCallback.calledOnce.should.be.true; });
  });

  it('path - virtual',
     () => Post.insert({source : 'foo.md', slug : 'bar'}).then(data => {
  ...
  it('permalink - should be encoded', () => {
    ...
    return Post.insert({source : 'foo.md', slug}).then(data => {
  ...
  it('permalink - trailing_index', () => {
    ...
    return Post.insert({source : 'foo.md', slug : 'bar/index.html'})
        .then(data => {
  ...
  it('permalink - trailing_html', () => {
    ...
    return Post.insert({source : 'foo.md', slug : 'bar/foo.html'})
        .then(data => {
          data.permalink.should.eql(hexo.config.url + '/' +
  ...
  it('asset_dir - virtual',
     () => Post.insert({source : 'foo.md', slug : 'bar'}).then(data => {
       data.asset_dir.should.eql(pathFn.join(hexo.source_dir, 'foo') +
                                 pathFn.sep);
       return Post.removeById(data._id);
     }));

  it('tags - virtual',
     () => Post.insert({source : 'foo.md', slug : 'bar'})
               .then(post => post.setTags([ 'foo', 'bar', 'baz' ])
                                 .thenReturn(Post.findById(post._id)))
               .then(post => {
                 post.tags.map(tag => tag.name).should.have.members([
                   'bar', 'baz', 'foo'
                 ]);
  ...
  it('setTags() - old tags should be removed', () => {
    ...
    return Post.insert({source : 'foo.md', slug : 'foo'})
        .then(post => {
          id = post._id;
  ...
  it('setTags() - sync problem',
     () => Post.insert([
               ...
               .then(posts => Promise
                                  ...
                                  .thenReturn(posts))
               .then(posts => {
                 Tag.map(tag => tag.name).should.have.members([
                   'foo', 'bar', 'baz'
                 ]);
  ...
  it('setCategories() - shared category should be same', () => {
    ...
    return Post.insert({source : 'foo.md', slug : 'foo'})
        ...
        .then(() =>
                  Post.insert({source : 'bar.md', slug : 'bar'}).then(post => {
                    postIdB = post._id;
                    return post.setCategories([ 'foo', 'bar' ]);
                  }))
        .then(() => {
          ...
          postA.categories.map(cat => cat._id)
              .should.eql(postB.categories.map(cat => cat._id));

          return Promise.all(
              [ Post.removeById(postIdA), Post.removeById(postIdB) ]);
        });
  });
  ...
  it('setCategories() - category not shared should be different', () => {
    ...
    return Post.insert({source : 'foo.md', slug : 'foo'})
        ...
        .then(() => {
          ...
          postCategoriesA.forEach(
              catId => { postCategoriesB.should.not.include(catId); });

          postCategoriesB.forEach(
              catId => { postCategoriesA.should.not.include(catId); });
  ...
  it('setCategories() - multiple hierarchies',
     () => Post.insert({source : 'foo.md', slug : 'bar'})
               .then(post =>
                         post.setCategories([ [ 'foo', '', 'bar' ], '', 'baz' ])
                             .thenReturn(Post.findById(post._id)))
               .then(post => {
