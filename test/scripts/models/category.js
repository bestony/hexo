...
describe('Category', () => {
  ...
  it('slug - virtual', async () => {
    const data = await Category.insert({name : 'foo'});
  ...
  it('slug - category_map', async () => {
    hexo.config.category_map = {test : 'wat'};

    const data = await Category.insert({name : 'test'});
  ...
  it('slug - filename_case: 0', async () => {
    const data = await Category.insert({name : 'WahAHa'});
  ...
  it('slug - parent', async () => {
    let cat = await Category.insert({name : 'parent'});
    cat = await Category.insert({name : 'child', parent : cat._id});
    cat.slug.should.eql('parent/child');

    await Promise.all(
        [ Category.removeById(cat._id), Category.removeById(cat.parent) ]);
  });
  ...
  it('permalink - should be encoded', async () => {
    ...
    const data = await Category.insert({name : '字'});
  ...
  it('check whether a category exists', async () => {
    ...
    try {
      await Category.insert({name : 'foo'});
    } catch (err) {
  ...
  it('check whether a category exists (with parent)', async () => {
    let data = await Category.insert({name : 'foo'});
    data = await Category.insert({name : 'foo', parent : data._id});
    await Promise.all(
        [ Category.removeById(data._id), Category.removeById(data.parent) ]);
  });
