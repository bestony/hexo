...
describe('Tag', () => {
  ...
  it('register()', () => {
    ...
    return tag.render('{% test foo.bar | abcdef > fn(a, b, c) < fn() %}')
        .then(result => {
  ...
  it('register() - async', () => {
    ...
    return tag.render('{% test foo bar %}')
        .then(result => { result.should.eql('foo bar'); });
  });

  it('register() - block', () => {
    const tag = new Tag();

    tag.register('test', (args, content) => args.join(' ') + ' ' + content,
                 true);

    const str =
        [ '{% test foo bar %}', 'test content', '{% endtest %}' ].join('\n');

    return tag.render(str).then(
        result => { result.should.eql('foo bar test content'); });
  });
  ...
  it('register() - async block', () => {
    ...
    tag.register('test',
                 (args, content) =>
                     Promise.resolve(args.join(' ') + ' ' + content),
  ...
  it('register() - nested test', () => {
    ...
    const str = [
      '{% test %}', '123456', '  {% raw %}', '  raw', '  {% endraw %}',
      '  {% test %}', '  test', '  {% endtest %}', '789012', '{% endtest %}'
    ].join('\n');
  ...
  it('register() - nested async / async test', () => {
    ...
    const str = [
      '{% test %}', '123456', '  {% async %}', '  async', '  {% endasync %}',
      '789012', '{% endtest %}'
    ].join('\n');
  ...
  it('register() - strip indention', () => {
    ...
    return tag.render(str).then(
        result => { result.should.eql('test content'); });
  });
  ...
  it('register() - async callback', () => {
    ...
    tag.register(
        'test',
        (args, content, callback) => { callback(null, args.join(' ')); },
        {async : true});
  ...
  it('render() - context', () => {
    ...
    tag.register('test', function() { return this.foo; });

    return tag.render('{% test %}', {foo : 'bar'})
        .then(result => { result.should.eql('bar'); });
  });
