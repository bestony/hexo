...
describe('asset', () => {
  ...
  function newFile(options) {
    ...
    options.params = {renderable : options.renderable};
  ...
  it('asset - type: create', async () => {
    const file =
        newFile({path : 'foo.jpg', type : 'create', renderable : false});
  ...
  it('asset - type: create (when source path is configed to parent directory)',
     async () => {
       ...
       await process(file);
       const id =
           '../source/foo.jpg'; // The id should a relative path,because the
                                // 'lib/models/assets.js' use asset path by
                                // joining base path with "_id" directly.
       const asset = Asset.findById(id);
       asset._id.should.eql(id);
       asset.path.should.eql(file.path);
       asset.modified.should.be.true;
  ...
  it('asset - type: update', async () => {
    const file =
        newFile({path : 'foo.jpg', type : 'update', renderable : false});

    const id = 'source/' + file.path;

    await Promise.all([
      writeFile(file.source, 'test'),
      Asset.insert({_id : id, path : file.path, modified : false})
    ]);
    await process(file);
    const asset = Asset.findById(id);
  ...
  it('asset - type: skip', async () => {
    const file = newFile({path : 'foo.jpg', type : 'skip', renderable : false});
    ...
    await Promise.all([ Asset.removeById(id), unlink(file.source) ]);
  });

  it('asset - type: delete', async () => {
    const file =
        newFile({path : 'foo.jpg', type : 'delete', renderable : false});

    const id = 'source/' + file.path;

    await Asset.insert({_id : id, path : file.path});
    await process(file);
  ...
  it('page - type: create', async () => {
    ...
    const file =
        newFile({path : 'hello.swig', type : 'create', renderable : true});
    ...
    await Promise.all([ page.remove(), unlink(file.source) ]);
  });
  ...
  it('page - type: update', async () => {
    ...
    const file =
        newFile({path : 'hello.swig', type : 'update', renderable : true});
  ...
  it('page - type: delete', async () => {
    const file =
        newFile({path : 'hello.swig', type : 'delete', renderable : true});

    await Page.insert({source : file.path, path : 'hello.html'});
    await process(file);
  ...
  it('page - permalink', async () => {
    const body =
        [ 'title: "Hello world"', 'permalink: foo.html', '---' ].join('\n');
  ...
  it('page - permalink (with trailing slash)', async () => {
    const body =
        [ 'title: "Hello world"', 'permalink: foo/', '---' ].join('\n');
  ...
  it('page - set layout to false if output is not html', async () => {
    ...
    const file =
        newFile({path : 'test.yml', type : 'create', renderable : true});
  ...
  it('page - don\'t remove extension name', async () => {
    ...
    const file =
        newFile({path : 'test.min.js', type : 'create', renderable : true});
