...
describe('post', () => {
  ...
  it('pattern', () => {
    // Renderable files
    pattern.match('_posts/foo.html')
        .should.eql({published : true, path : 'foo.html', renderable : true});

    pattern.match('_drafts/bar.html')
        .should.eql({published : false, path : 'bar.html', renderable : true});

    // Non-renderable files
    pattern.match('_posts/foo.txt')
        .should.eql({published : true, path : 'foo.txt', renderable : false});

    pattern.match('_drafts/bar.txt')
        .should.eql({published : false, path : 'bar.txt', renderable : false});
    ...
    pattern.match('_posts/foo/bar.html')
  ...
  it('asset - type: create', () => {
    ...
    return Promise
        ...
        .finally(() => {
          ...
          return Promise.all(
              [ Post.removeById(postId), fs.unlink(file.source) ]);
        });
  });
    ...
    return Promise
        ...
        .spread(post => {
          ...
          return PostAsset.insert(
              {_id : id, slug : file.path, modified : false, post : postId});
        })
        .then(() => process(file))
        .then(() => {
          const asset = PostAsset.findById(id);
  ...
  it('asset - type: delete', () => {
    ...
    return Post.insert({source : '_posts/foo.html', slug : 'foo'})
        .then(post => {
          postId = post._id;
        ...
        .then(() => process(file))
        .then(() => { should.not.exist(PostAsset.findById(id)); })
        .finally(() => {
          hexo.config.post_asset_folder = false;
  ...
  it('asset - skip if can\'t find a matching post', () => {
    ...
    return fs.writeFile(file.source, 'test')
        .then(() => process(file))
        .then(() => { should.not.exist(PostAsset.findById(id)); })
        .finally(() => {
          hexo.config.post_asset_folder = false;
  ...
  it('asset - the related post has been deleted', () => {
    ...
    return Promise
        .all([
          fs.writeFile(file.source, 'test'),
          PostAsset.insert({_id : id, slug : file.path})
        ])
        .then(() => process(file))
        .then(() => { should.not.exist(PostAsset.findById(id)); })
        .finally(() => {
  ...
  it('post - type: create', () => {
    const body = [
      'title: "Hello world"', 'date: 2006-01-02 15:04:05',
    ...
    return fs.writeFile(file.source, body)
        .then(() => process(file))
        .then(() => {
  ...
  it('post - type: delete', () => {
    ...
    return Post.insert({source : file.path, slug : 'foo'})
        .then(() => process(file))
        .then(() => { should.not.exist(Post.findOne({source : file.path})); });
  });
  ...
  it('post - published', () => {
    const body =
        [ 'title: "Hello world"', 'published: false', '---' ].join('\n');
  ...
  it('post - always set published: false for drafts', () => {
    const body =
        [ 'title: "Hello world"', 'published: true', '---' ].join('\n');
  ...
  it('post - use the status of the source file if date not set', () => {
    ...
    return fs.writeFile(file.source, body)
        ...
        .spread(stats => {
          ...
          post.date.toDate().setMilliseconds(0).should.eql(
              stats.birthtime.setMilliseconds(0));
          post.updated.toDate().setMilliseconds(0).should.eql(
  ...
  it('post - photo is an alias for photos', () => {
    const body = [
      'title: "Hello world"', 'photo:', '- https://hexo.io/foo.jpg',
    ...
    return fs.writeFile(file.source, body)
        ...
        .then(() => {
          ...
          post.photos.should.eql(
              [ 'https://hexo.io/foo.jpg', 'https://hexo.io/bar.png' ]);
  ...
  it('post - photos (not array)', () => {
    const body = [
      'title: "Hello world"', 'photos: https://hexo.io/foo.jpg', '---'
    ].join('\n');
  ...
  it('post - category is an alias for categories', () => {
    const body =
        [ 'title: "Hello world"', 'category:', '- foo', '- bar', '---' ].join(
            '\n');
  ...
  it('post - categories (not array)', () => {
    const body =
        [ 'title: "Hello world"', 'categories: foo', '---' ].join('\n');
  ...
  it('post - categories (multiple hierarchies)', () => {
    const body = [
      'title: "Hello world"', 'categories:', '- foo', '- [bar, baz]', '---'
    ].join('\n');
    ...
    return fs.writeFile(file.source, body)
        ...
        .then(() => {
          ...
          post.categories.map(item => item.name).should.eql([
            'foo', 'bar', 'baz'
          ]);
  ...
  it('post - tag is an alias for tags', () => {
    const body =
        [ 'title: "Hello world"', 'tags:', '- foo', '- bar', '---' ].join('\n');
    ...
    return fs.writeFile(file.source, body)
        ...
        .then(() => {
          ...
          post.tags.map(item => item.name).should.have.members([
            'foo', 'bar'
          ]);
  ...
  it('post - post_asset_folder disabled', () => {
    ...
    return Promise
        .all([ fs.writeFile(file.source, ''), fs.writeFile(assetPath, '') ])
        .then(() => process(file))
        .then(() => {
          const post = Post.findOne({source : file.path});
        ...
        .finally(() => Promise.all(
                     [ fs.unlink(file.source), fs.unlink(assetPath) ]));
  });

  it('post - parse date', () => {
    const body = [
      'title: "Hello world"', 'date: Apr 24 2014', 'updated: May 5 2015', '---'
    ].join('\n');
  ...
  it('post - use file stats instead if date is invalid', () => {
    const body = [
      'title: "Hello world"', 'date: yomama', 'updated: isfat', '---'
    ].join('\n');
  ...
  it('post - timezone', () => {
    const body = [
      'title: "Hello world"', 'date: 2014-04-24', 'updated: 2015-05-05', '---'
    ].join('\n');
  ...
  it('post - permalink', () => {
    const body =
        [ 'title: "Hello world"', 'permalink: foooo', '---' ].join('\n');
