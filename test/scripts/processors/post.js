...
describe('post', () => {
  ...
  it('asset - type: create', async () => {
    ...
    const doc = await Post.insert({source : '_posts/foo.html', slug : 'foo'});
    ...
    await Promise.all([ Post.removeById(postId), unlink(file.source) ]);
  });
  ...
  it('asset - type: update', async () => {
    ...
    const post = await Post.insert({source : '_posts/foo.html', slug : 'foo'});
    await writeFile(file.source, 'test');
  ...
  it('post - type: delete', async () => {
    ...
    await Post.insert({source : file.path, slug : 'foo'});
    await process(file);
  ...
  it('post - post_asset_folder enabled', async () => {
    ...
    await Promise.all(
        [ unlink(file.source), ...assetFiles.map(obj => unlink(obj.path)) ]);
  });
  ...
  it('post - post_asset_folder disabled', async () => {
    ...
    await Promise.all([ writeFile(file.source, ''), writeFile(assetPath, '') ]);
    await process(file);
    const post = Post.findOne({source : file.path});
    should.not.exist(PostAsset.findById(assetId));

    post.remove();
    await Promise.all([ unlink(file.source), unlink(assetPath) ]);
  });
