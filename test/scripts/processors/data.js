...
describe('data', () => {
  ...
  it('pattern', () => {
    ...
    pattern.match('_data/users.json')
        .should.eql(
            {0 : '_data/users.json', 1 : 'users.json', path : 'users.json'});

    pattern.match('_data/users.yaml')
        .should.eql(
            {0 : '_data/users.yaml', 1 : 'users.yaml', path : 'users.yaml'});
  ...
  it('type: create - yaml', async () => {
    ...
    const file = newFile({path : 'users.yml', type : 'create'});
  ...
  it('type: create - json', async () => {
    ...
    const file = newFile({path : 'users.json', type : 'create'});
  ...
  it('type: create - others', async () => {
    const file = newFile({path : 'users.txt', type : 'create'});
  ...
  it('type: update', async () => {
    ...
    const file = newFile({path : 'users.yml', type : 'update'});

    await Promise.all([
      writeFile(file.source, body), Data.insert({_id : 'users', data : {}})
    ]);
    await process(file);
    const data = Data.findById('users');
  ...
  it('type: delete', async () => {
    const file = newFile({path : 'users.yml', type : 'delete'});

    await Data.insert({_id : 'users', data : {foo : 'bar'}});
    await process(file);
