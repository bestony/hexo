...
describe('feed_tag', () => {
  const ctx = {
    config :
        {title : 'Hexo', url : 'http://yoursite.com', root : '/', feed : {}}
  };
  ...
  it('type - null', () => {
    feed('foo.xml', {
      type : null
    }).should.eql('<link rel="alternate" href="/foo.xml" title="Hexo" >');
  });
  ...
  it('feed - parse argument if available', () => {
    ctx.config.feed = {type : 'atom', path : 'atom.xml'};

    feed('atomic.xml')
        .should.eql(
            '<link rel="alternate" href="/atomic.xml" title="Hexo" type="application/atom+xml">');
  });
  ...
  it('feed - rss2', () => {
    ctx.config.feed = {type : 'rss2', path : 'rss.xml'};
