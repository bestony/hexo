...
describe('full_url_for', () => {
  const ctx = {config : {url : 'https://example.com'}};

  const fullUrlFor =
      require('../../../lib/plugins/helper/full_url_for').bind(ctx);

  it('no path input', () => { fullUrlFor().should.eql(ctx.config.url + '/'); });
  ...
  it('external url', () => {
    ['https://hexo.io/', '//google.com/',
     ...
     '//google.com/index.html']
        .forEach(url => { fullUrlFor(url).should.eql(url); });
  });
