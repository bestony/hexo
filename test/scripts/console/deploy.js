...
describe('deploy', () => {
  ...
  it('single deploy setting', () => {
    hexo.config.deploy = {type : 'foo', foo : 'bar'};

    const deployer = sinon.spy(
        args => { args.should.eql({type : 'foo', foo : 'foo', bar : 'bar'}); });
  ...
  it('multiple deploy setting', () => {
    const deployer1 = sinon.spy(
        args => { args.should.eql({type : 'foo', foo : 'foo', test : true}); });

    const deployer2 = sinon.spy(
        args => { args.should.eql({type : 'bar', bar : 'bar', test : true}); });

    hexo.config.deploy =
        [ {type : 'foo', foo : 'foo'}, {type : 'bar', bar : 'bar'} ];
  ...
  it('run generate if public directory not exist',
     () => fs.rmdir(hexo.public_dir)
               ...
               .then(exist => { exist.should.be.true; }));
});
