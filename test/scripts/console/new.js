...
describe('new', () => {
  ...
  it('title', async () => {
    ...
    const body =
        [
          'title: Hello World', 'date: ' + date.format('YYYY-MM-DD HH:mm:ss'),
          'tags:', '---'
        ].join('\n') +
        '\n';

    await n({_ : [ 'Hello World' ]});
  ...
  it('slug', async () => {
    ...
    await n({_ : [ 'Hello World' ], slug : 'foo'});
    const content = await readFile(path);
    content.should.eql(body);
  ...
  it('path', async () => {
    ...
    await n({_ : [ 'Hello World' ], slug : 'foo', path : 'bar'});
    const content = await readFile(path);
    content.should.eql(body);
  ...
  it('path - p', async () => {
    ...
    await n({_ : [ 'Hello World' ], slug : 'foo', p : 'bar'});
    const content = await readFile(path);
    content.should.eql(body);
  ...
  it('rename if target existed', async () => {
    ...
    await Promise.all([
      unlink(path), unlink(join(hexo.source_dir, '_posts', 'Hello-World.md'))
    ]);
  });
  ...
  it('replace existing files', async () => {
    ...
    await post.create({title : 'Hello World'});
    await n({_ : [ 'Hello World' ], replace : true});
    const exist =
        await exists(join(hexo.source_dir, '_posts', 'Hello-World-1.md'));
  ...
  it('extra data', async () => {
    ...
    const body =
        [
          ...
          'date: ' + date.format('YYYY-MM-DD HH:mm:ss'), 'tags:', '---'
        ].join('\n') +
        '\n';

    await n({_ : [ 'Hello World' ], foo : 'bar'});
    const content = await readFile(path);
    content.should.eql(body);
