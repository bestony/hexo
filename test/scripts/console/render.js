...
describe('render', () => {
  ...
  it('relative path', () => {
    ...
    return fs.writeFile(src, body)
        ...
        .then(result => {
          JSON.parse(result).should.eql({foo : 1, bar : {boo : 2}});
  ...
  it('engine', () => {
    ...
    return fs.writeFile(src, body)
        .then(() => render(
                  {_ : [ 'test' ], output : 'result.json', engine : 'yaml'}))
        .then(() => fs.readFile(dest))
        .then(result => {
          JSON.parse(result).should.eql({foo : 1, bar : {boo : 2}});
  ...
  it('pretty', () => {
    ...
    return fs.writeFile(src, body)
        .then(() => render(
                  {_ : [ 'test.yml' ], output : 'result.json', pretty : true}))
        .then(() => fs.readFile(dest))
        .then(result => {
          result.should.eql(
              JSON.stringify({foo : 1, bar : {boo : 2}}, null, '  '));
