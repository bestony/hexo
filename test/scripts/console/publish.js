...
describe('publish', () => {
  ...
  beforeEach(() => post.create({title : 'Hello World', layout : 'draft'}));
  ...
  it('layout', async () => {
    ...
    const content = [
      '---', 'layout: photo', 'title: Hello World',
      'date: ' + date.format('YYYY-MM-DD HH:mm:ss'), 'tags:', '---'
    ].join('\n') + '\n';

    await publish({_ : [ 'photo', 'Hello-World' ]});
    const data = await readFile(path);
    data.should.eql(content);
  ...
  it('replace existing target', async () => {
    ...
    await post.create({title : 'Hello World'});
    await publish({_ : [ 'Hello-World' ], replace : true});
    const exist =
