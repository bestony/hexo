  ...
  _parseArgs(parser, nodes, lexer) {
    ...
    while ((token = parser.nextToken(true))) {
      if (token.type === lexer.TOKEN_WHITESPACE ||
        ...
        if (argitem !== '') {
          const argnode =
              new nodes.Literal(tag.lineno, tag.colno, argitem.trim());
...
const trimBody =
    body => { return stripIndent(body()).replace(/^\n?|\n?$/g, ''); };
...
class NunjucksAsyncBlock extends NunjucksBlock {
  ...
  run(context, args, body, callback) {
    ...
    body((err, result) => {
      ...
      this._run(context, args, trimBody(body))
...
class Tag {
  ...
  render(str, options, callback) {
    ...
    return Promise
        .fromCallback(cb => { this.env.renderString(str, options, cb); })
        .catch(err => Promise.reject(formatNunjucksError(err, str)))
        .then(result =>
                  result.replace(rPlaceholder, (_, index) => cache[index]));
  }
}
