...
module.exports = ctx => {
  ...
  Category.virtual('path').get(function() {
    ...
    if (catDir === '/')
      catDir = '';
    if (catDir.length && catDir[catDir.length - 1] !== '/')
      catDir += '/';

    return `${catDir + this.slug}/`;
  });

  Category.virtual('permalink')
  ...
  Category.virtual('posts').get(function() {
    ...
    const ids =
        PostCategory.find({category_id : this._id}).map(item => item.post_id);

    return ctx.locals.get('posts').find({_id : {$in : ids}});
  });

  Category.virtual('length').get(function() { return this.posts.length; });
  ...
  Category.pre('save', data => {
    ...
    const cat = Category.findOne({name, parent : parent || { $exists: false }},
                                 {lean : true});
