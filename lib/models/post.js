...
function pickID(data) { return data._id; }
...
module.exports = ctx => {
  ...
  Post.virtual('permalink')
      .get(function() { return full_url_for.call(ctx, this.path); });

  Post.virtual('full_source')
      .get(function() { return join(ctx.source_dir, this.source || ''); });
  ...
  Post.virtual('tags').get(function() {
    ...
    const ids = PostTag.find({post_id : this._id}, {lean : true})
  ...
  Post.method('setTags', function(tags) {
    ...
    return Promise
        ...
        .map(tag => {
          // Find the reference
          const ref =
              PostTag.findOne({post_id : id, tag_id : tag._id}, {lean : true});
          if (ref)
            return ref;

          // Insert the reference if not exist
          return PostTag.insert({post_id : id, tag_id : tag._id});
        })
        .then(tags => {
          // Remove old tags
          const deleted =
              existed.filter(item => !tags.map(pickID).includes(item));
          return deleted;
        })
  ...
  Post.virtual('categories').get(function() {
    ...
    const ids = PostCategory.find({post_id : this._id}, {lean : true})
  ...
  Post.method('setCategories', function(cats) {
    ...
    const allIds = [];
    const existed =
        PostCategory.find({post_id : id}, {lean : true}).map(pickID);
    ...
    const addHierarchy = catHierarchy => {
      ...
      return Promise.each(catHierarchy, (cat, i) => {
        ...
        const data = Category.findOne(
            {name : cat, parent : i ? parentIds[i - 1] : {$exists : false}},
            {lean : true});
    ...
    return (hasHierarchy ? Promise.each(cats, addHierarchy)
                         : Promise.resolve(addHierarchy(cats)))
        .then(() => allIds)
        .map(catId => {
          // Find the reference
          const ref = PostCategory.findOne({post_id : id, category_id : catId},
                                           {lean : true});
          if (ref)
            return ref;

          // Insert the reference if not exist
          return PostCategory.insert({post_id : id, category_id : catId});
        })
