...
const {copyDir, exists, listDir, mkdirs, readFile, rmdir, unlink, writeFile} =
    require('hexo-fs');
...
class PostRenderCache {
  ...
  escapeContent(str) {
    ...
    return str.replace(rEscapeContent,
                       (_, content) => _escapeContent(this.cache, content));
  }
...
const removeExtname =
    str => { return str.substring(0, str.length - extname(str).length); };
...
class Post {
  ...
  _renderScaffold(data) {
    ...
    return this._getScaffold(data.layout)
        ...
        .then(frontMatter => {
          ...
          // Parse front-matter
          const obj =
              jsonMode ? JSON.parse(`{${frontMatter}}`) : load(frontMatter);
          ...
          if (yfmSplit.prefixSeparator)
            content += `${separator}\n`;

          content += yfm.stringify(obj, {mode : jsonMode ? 'json' : ''});
  ...
  publish(data, replace, callback) {
    ...
    const slug =
        slugize(data.slug.toString(), {transform : config.filename_case});
    data.slug = slug;
    ...
    return listDir(draftDir)
        .then(list => { return list.find(item => regex.test(item)); })
        .then(item => {
          if (!item)
  ...
  render(source, data = {}, callback) {
    ...
    if (data.content != null) {
      ...
      return Promise.reject(new Error('No input file or string!'))
          .asCallback(callback);
    }
    ...
    const disableNunjucks = ext && ctx.render.renderer.get(ext) &&
