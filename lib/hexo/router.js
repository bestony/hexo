    ...
    data()
        .then(data => {
          if (data instanceof Stream && data.readable) {
            data.on('data', d => { this.push(d); });

            data.on('end', () => { this.push(null); });

            data.on('error', err => { this.emit('error', err); });
...
const _format = path => {
  ...
  path = path.replace(/^\/+/, '')   // Remove prefixed slashes
...
class Router extends EventEmitter {
  ...
  format(path) { return _format(path); }
