...
class Hexo {
  constructor(base = process.cwd(), args = {}) {
    ...
    this.database =
        new Database({version : dbVersion, path : join(dbPath, 'db.json')});
  ...
  _bindLocals() {
    ...
    locals.set('data', () => {
      ...
      db.model('Data').forEach(data => { obj[data._id] = data.data; });
  ...
  call(name, args, callback) {
    ...
    return new Promise((resolve, reject) => {
             ...
             if (c) {
               ...
               reject(new Error(
                   `Console \`${name}\` has not been registered yet!`));
             }
           })
        .asCallback(callback);
  }

  model(name, schema) { return this.database.model(name, schema); }
  ...
  resolvePlugin(name) {
    ...
    try {
      ...
      // There was an error (likely the plugin wasn't found), so return a
      // possibly non-existing path that a later part of the resolution process
      // will check.
  ...
  loadPlugin(path, callback) {
    return readFile(path)
        .then(script => {
          // Based on:
          // https://github.com/joyent/node/blob/v0.10.33/src/node.js#L516
          const module = new Module(path);
          module.filename = path;
          module.paths = Module._nodeModulePaths(path);

          function require(path) { return module.require(path); }
  ...
  load(callback) {
    return loadDatabase(this)
        ...
        .then(() => this._generate({cache : false}))
        .asCallback(callback);
  }

  watch(callback) {
    let useCache = false;
    const {cache} = Object.assign({cache : false}, this.config.server);
  ...
  _runGenerators() {
    ...
    return Promise
        ...
        .reduce(
            (result, data) => { return data ? result.concat(data) : result; },
            []);
  }
  ...
  _routerReflesh(runningGenerators, useCache) {
    ...
    return runningGenerators
        .map(generatorResult => {
          if (typeof generatorResult !== 'object' ||
          ...
          return this
              .execFilter('template_locals', new Locals(path, data),
                          {context : this})
              .then(locals => {
                route.set(path,
        ...
        .then(newRouteList => {
          // Remove old routes
          const removed =
              routeList.filter(item => !newRouteList.includes(item));
  ...
  _generate(options = {}) {
    ...
    return this
        .execFilter('before_generate', this.locals.get('data'),
                    {context : this})
  ...
  exit(err) {
    if (err) {
      this.log.fatal(
          {err}, 'Something\'s wrong. Maybe you can find the solution here: %s',
          underline('https://hexo.io/docs/troubleshooting.html'));
    }

    return this.execFilter('before_exit', null, {context : this})
        .then(() => { this.emit('exit', err); });
  }
