...
function Box(ctx, base, options) {
  ...
  this.options = Object.assign({persistent : true}, options);
  ...
  if (ctx.config.ignore) {
    if (ctx.config.ignore.length)
      ignoreCfg = ignoreCfg.concat(ctx.config.ignore);
  }
  this.ignore = ignoreCfg;
  const targets = ignoreCfg;
  this.options.ignored =
      (this.options.ignored ||
       []).concat(targets.map(s => toRegExp(ctx, s)).filter(x => x));
}
...
function toRegExp(ctx, arg) {
  ...
  if (typeof arg !== 'string') {
    ctx.log.warn(
        'A value of "ignore:" section in "_config.yml" is not invalid (not a string)');
    return null;
  }
  const result = makeRe(arg);
  if (!result) {
    ctx.log.warn(
        'A value of "ignore:" section in "_config.yml" can not be converted to RegExp:' +
        arg);
    return null;
  }
  return result;
...
Box.prototype._createFileClass = function() {
  ...
  const _File = function(data) { Reflect.apply(File, this, [ data ]); };
  ...
  _File.prototype.renderSync = function(
      options) { return ctx.render.renderSync({path : this.source}, options); };
...
Box.prototype.addProcessor = function(pattern, fn) {
  ...
  this.processors.push({pattern, process : fn});
};
...
Box.prototype._readDir = function(base, fn, prefix = '') {
  ...
  return readdir(base)
      .map(path => stat(join(base, path)).then(stats => {
        ...
        return this._checkFileStatus(prefix + path)
            .then(file => fn(file).thenReturn(file));
      }))
      .catch(err => {
...
Box.prototype._checkFileStatus = function(path) {
  ...
  return Cache
      .compareFile(escapeBackslash(src.substring(ctx.base_dir.length)),
                   () => getHash(src),

                   () => stat(src))
      .then(result => ({type : result.type, path}));
};
...
Box.prototype.process = function(callback) {
  ...
  return stat(base)
      .then(stats => {
        ...
        const relativeBase =
            escapeBackslash(base.substring(ctx.base_dir.length));
        const cacheFiles =
            Cache.filter(item => item._id.startsWith(relativeBase))
                .map(item => item._id.substring(relativeBase.length));

        // Read files from directory
        return this
            ._readDir(base, file => this._processFile(file.type, file.path))
            .map(file => file.path)
            .then(files => // Handle deleted files
                  Promise.filter(cacheFiles, path => !files.includes(path))
  ...
  return Promise
      .reduce(this.processors,
              (count, processor) => {
                ...
                const file =
                    new File({source : join(base, path), path, params, type});

                return Reflect
                    .apply(Promise.method(processor.process), ctx, [ file ])
                    .thenReturn(count + 1);
              },
              0)
      .then(count => {
      ...
      .catch(
          err => { ctx.log.error({err}, 'Process failed: %s', magenta(path)); })
      .finally(() => { this._processingFiles[path] = false; })
      .thenReturn(path);
};
...
Box.prototype.watch = function(callback) {
  ...
  return this.process()
      ...
      .then(watcher => {
        ...
        watcher.on(
            'add',
            path => { this._processFile(File.TYPE_CREATE, getPath(path)); });

        watcher.on(
            'change',
            path => { this._processFile(File.TYPE_UPDATE, getPath(path)); });

        watcher.on(
            'unlink',
            path => { this._processFile(File.TYPE_DELETE, getPath(path)); });
        ...
        watcher.on('addDir', path => {
          ...
          this._readDir(path, file => this._processFile(file.type, file.path),
                        prefix);
        });
      })
      .asCallback(callback);
...
Box.prototype.isWatching = function() { return Boolean(this.watcher); };
