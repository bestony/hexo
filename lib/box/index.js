...
function Box(ctx, base, options) {
  ...
  this.options.ignored = (this.options.ignored || []).concat(
    targets.map(s => toRegExp(ctx, s)).filter(x => x)
  );
}
...
Box.prototype._createFileClass = function() {
  ...
  _File.prototype.render = function(options, callback) {
    ...
    return ctx.render
      .render({ path: this.source }, options)
      .asCallback(callback);
  };
...
Box.prototype._readDir = function(base, fn, prefix = "") {
  ...
  return readdir(base)
    .map(path =>
      stat(join(base, path)).then(stats => {
...
Box.prototype._processFile = function(type, path) {
  ...
  return Promise.reduce(
    ...
    (count, processor) => {
      ...
      const file = new File({ source: join(base, path), path, params, type });
