...
const {toDate, timezone, isExcludedFile, isTmpFile, isHiddenFile, isMatch} =
    require('./common');
...
module.exports = ctx => {
  function processPost(file) {
    ...
    return Promise.all([ file.stat(), file.read() ])
        .spread((stats, content) => {
          ...
          } else if (info && info.year && (info.month || info.i_month) &&
                     (info.day || info.i_day)) {
            data.date = new Date(info.year,
          ...
          // FIXME: Data may be inserted when reading files. Load it again to
          // prevent race condition. We have to solve this in warehouse.
          const doc = Post.findOne({source : file.path});
        ...
        .then(doc => Promise.all([
          doc.setCategories(categories), doc.setTags(tags), scanAssetDir(doc)
        ]));
  }
  ...
  function scanAssetDir(post) {
    ...
    return fs.stat(assetDir)
        ...
        .map(item => {
          const id =
              join(assetDir, item).substring(baseDirLength).replace(/\\/g, '/');
          const asset = PostAsset.findById(id);
          if (asset)
            return undefined;

          return PostAsset.save(
              {_id : id, post : post._id, slug : item, modified : true});
        });
  }
  ...
  function processAsset(file) {
    ...
    const post =
        Post.toArray().find(post => file.source.startsWith(post.asset_dir));
  ...
  return {
    pattern : new Pattern(path => {
      ...
      if (path.startsWith(postDir)) {
        result = {published : true, path : path.substring(postDir.length)};
      } else if (path.startsWith(draftDir)) {
        result = {published : false, path : path.substring(draftDir.length)};
      }
      ...
      result.renderable = ctx.render.isRenderable(path) &&
                          !isMatch(path, ctx.config.skip_render);
      return result;
    }),
