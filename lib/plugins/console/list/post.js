...
function mapName(item) { return item.name; }
...
function listPost() {
  ...
  const data = Post.sort({published : -1, date : 1}).map(post => {
    ...
    return [
      gray(date), post.title, magenta(post.source), categories.join(', '),
      tags.join(', ')
    ];
  });

  // Table header
  const header = [ 'Date', 'Title', 'Path', 'Category', 'Tags' ].map(
      str => underline(str));

  data.unshift(header);

  const t = table(data, {stringLength});
