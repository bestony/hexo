...
function generateConsole(args = {}) {
  ...
  function writeFile(path, force) {
    ...
    return pipeStream(dataStream, cacheStream, hashStream)
        .then(() => {
          ...
          return Cache.save({_id : cacheId, hash})
  ...
  function wrapDataStream(dataStream, options) {
    ...
    dataStream.on('error', err => { log.error(err); });
  ...
  function firstGenerate() {
    ...
    return fs.stat(publicDir)
        .then(stats => {
          if (!stats.isDirectory()) {
            throw new Error('%s is not a directory',
                            magenta(tildify(publicDir)));
          }
        })
        .catch(err => {
        ...
        .then(() => {
          ...
          const publicFiles =
              Cache.filter(item => item._id.startsWith('public/'))
          ...
          return Promise.all(Promise.map(
              tasks, doTask,
  ...
  if (args.w || args.watch) {
    return this.watch().then(firstGenerate).then(() => {
      ...
      route
          ...
          .on('remove', path => { deleteFile(path); });
    });
  }
