...
function deployConsole(args) {
  ...
  if (!config) {
    ...
    help += `For more help, you can check the online docs: ${
        underline('https://hexo.io/')}`;
  ...
  return new Promise((resolve, reject) => {
      ...
      .each(item => {
        ...
        return Reflect
            .apply(deployers[type], this, [ Object.assign({}, item, args) ])
            .then(() => { this.log.info('Deploy done: %s', magenta(type)); });
      })
      .then(() => { this.emit('deployAfter'); });
}
