...
// Based on:
// https://raw.github.com/imathis/octopress/master/plugins/code_block.rb
 ...
 * @param {Object} first_line Specify the first line number, value must be a
 *     number
 * @param {Object} mark Line highlight specific line(s), each value separated by
 *     a comma. Specify number range using a dash
...
function getHighlightOptions(config, arg) {
  ...
  if (rCaptionUrlTitle.test(arg)) {
    ...
    caption =
        `<span>${match[1]}</span><a href="${match[2]}${match[3]}">link</a>`;
