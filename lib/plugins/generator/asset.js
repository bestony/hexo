...
const process = (name, ctx) => {
  return Promise
      .filter(ctx.model(name).toArray(),
              asset => fs.exists(asset.source).tap(exist => {
                if (!exist)
                  return asset.remove();
              }))
      .map(asset => {
        ...
        if (asset.renderable && ctx.render.isRenderable(path)) {
          ...
          data.data = () =>
              ctx.render.render({path : source, toString : true}).catch(err => {
...
function assetGenerator(locals) {
  return Promise.all([ process('Asset', this), process('PostAsset', this) ])
