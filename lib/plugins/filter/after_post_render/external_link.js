...
function externalLinkFilter(data) {
  ...
  if (
    typeof config.external_link === "undefined" ||
    typeof config.external_link === "object" ||
    config.external_link === true
  ) {
    config.external_link = Object.assign(
      { enable: true, field: "site", exclude: [] },
      config.external_link
    );
  }
  if (
    config.external_link === false ||
    config.external_link.enable === false ||
  ...
  data.content = data.content.replace(
    ...
    (str, _, href) => {
      if (
        /target=/gi.test(str) ||
        !isExternalLink(href, config.url, config.external_link.exclude)
      )
        return str;
