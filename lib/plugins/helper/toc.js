...
function tocHelper(str, options = {}) {
  options = Object.assign(
      {min_depth : 1, max_depth : 6, class : 'toc', list_number : true},
      options);

  const data = tocObj(
      str, {min_depth : options.min_depth, max_depth : options.max_depth});
  ...
  for (let i = 0, len = data.length; i < len; i++) {
    ...
    result += htmlTag(
        'li', {class : `${className}-item ${className}-level-${level}`});
