...
function feedTagHelper(path, options = {}) {
  ...
  if (path) {
    ...
    return `<link rel="alternate" href="${this.url_for(
      path
    )}" title="${title}" ${typeAttr}>`;
  }
  ...
  if (!path && config.feed) {
    ...
    if (feed.type && feed.path) {
      if (typeof feed.type === "string") {
        return `<link rel="alternate" href="${this.url_for(
          feed.path
        )}" title="${title}" type="application/${feedFn(feed.type)}+xml">`;
      }

      let result = "";
      for (const i in feed.type) {
        result += `<link rel="alternate" href="${this.url_for(
          feed.path[i]
        )}" title="${title}" type="application/${feedFn(feed.type[i])}+xml">`;
      }
      return result;
    }
  }
