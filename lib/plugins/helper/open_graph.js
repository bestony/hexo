...
function meta(name, content) { return `${htmlTag('meta', {name, content})}\n`; }
...
function openGraphHelper(options = {}) {
  ...
  let description = options.description || page.description || page.excerpt ||
  ...
  if (language) {
    if (language.length === 2) {
      ...
      language = language.replace('-', '_').replace(territoryRegex,
  ...
  images.forEach(path => { result += og('og:image', path); });
  ...
  if (keywords) {
    ...
    keywords.map(tag => { return tag.name ? tag.name : tag; })
        .filter(Boolean)
        .forEach(keyword => { result += og('article:tag', keyword); });
  }
